...
users.forEach(function(elem){
  sum = sum + `
    <div class="card">
        <img class="cover-photo" src="${elem.coverPhoto}" alt="Cover Photo"> 
        <img class="profile-photo" src="${elem.profilePhoto}" alt="Profile Picture">
        <img class="blue-tick" src="https://www.pngmart.com/files/12/Twitter-Verified-Badge-PNG-Image.png" alt="blue tick">
        <h1>${elem.username}</h1>
        <h4 class="name">${elem.fullName}</h4>
        <p>${elem.description}</p> 
        <div class="container">
            <div class="linee"></div>
        </div>
        <div class="line"> 
            <h4>${elem.mediaUploadedCount}k <br><span>Media</span></h4>
            <h4>${elem.followersCount}k<br> <span>followers</span></h4>
            <h4>${elem.followingCount} <br><span>following</span></h4>
        </div>
    </div>`
})
